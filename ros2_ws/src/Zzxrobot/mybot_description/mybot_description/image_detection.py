...
class ImageDisplayNode(Node):
    ...
    def __init__(self):
        ...
        self.publisher_nav = self.create_publisher(
            Float32MultiArray, 'target_position', 30)        # 创建话题，告知导航节点目标位置
        self.publisher_wheel = self.create_publisher(
            String, 'wheel_control', 10)        # 创建话题，控制差速节点进行位姿调节
        # 初始化 CV Bridge
        self.br = CvBridge()
        ...
        model_path = os.path.join(current_dir,
                                  '../../../../share/mybot_description/weights/yolov8l.pt')
    ...
    def nav_callback(self, msg):
        if msg.data == 'correct':           # 校准操作，启动小车令其转动
            ...
            self.get_logger().info(
                f"任务已更新: {self.task} ，类别：{self.model.names[self.target]}-{self.target}")
            ...
            for box in boxes:                                   # 获得深度（目标距离）并绘制
                ...
                if int(box.cls) == self.target and not np.isnan(depth_value):
                    if not self.correcting and not self.send_target:        # 排除校准位姿或已经发送过的情况
                        ...
                        centered = abs(
                            (x1 + x2) // 2 - self.camera_cx
                        ) < 15
                        if centered and 0.0 < depth_value < 1.0:
                            self.publisher_wheel.publish(
                                String(data=str(2)))                 # 距离偏远，小车前进
                        elif centered and depth_value < 0.0:
                            self.publisher_wheel.publish(
                                String(data=str(0)))                 # 停止小车
                            self.publisher_nav.publish(
                                Float32MultiArray(data=[1.0]))         # 告知导航节点可以抓取
        ...
        try:
            ...
            self.tf_buffer.can_transform(
                ...
                timeout=rclpy.duration.Duration(
                    seconds=1.0))
            # 执行转换
            point_base_link = self.tf_buffer.transform(
                camera_point,
                'base_link',
                timeout=rclpy.duration.Duration(
                    seconds=1.0))
