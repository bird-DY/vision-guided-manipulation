...
class NavigationNode(Node):
    def __init__(self):
        ...
        self.publisher_grasp = self.create_publisher(
            String, 'navigate_over', 10)        # 创建话题，告知抓取节点导航结束开始夹取
        self.publisher_vision = self.create_publisher(
            String, 'vision_on', 10)        # 创建话题，告知视觉节点下一个任务索引
    ...
    def target_callback(self, msg):
        ...
        if len(msg.data) == 1 and msg.data[0] == 1.0 and self.state == GET_TARGET:        # 校准位姿完成
            ...
            self.get_logger().info("当前状态: 已校准位姿")
            ...
            if int(msg.data[0]) == self.task and self.state == GET_AREA:
                ...
                self.get_logger().info("当前状态: 导航前往目标航点")
    ...
    def navigate_to_target(self, point, tf=False):       # tf为True需要转换
        ...
        try:
            ...
            yaw = math.atan2(point['y'] - transform.transform.translation.y,
